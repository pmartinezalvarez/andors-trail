...
public final class ToolboxView extends LinearLayout implements OnClickListener {
	...
	private final ImageButton toolbox_twinsprite;
	...
	public ToolboxView(final Context context, AttributeSet attrs) {
		...
		toolbox_twinsprite = (ImageButton) findViewById(R.id.toolbox_twinsprite);
		toolbox_twinsprite.setOnClickListener(this);
		toolbox_combatlog = (ImageButton) findViewById(R.id.toolbox_combatlog);
		toolbox_combatlog.setOnClickListener(this);

		Resources res = getResources();
		quickSlotIconsLockedDrawable = new LayerDrawable(new Drawable[] { res.getDrawable(quickSlotIcon),
	...
	public void onClick(View btn) {
		...
		if (btn == toggleToolboxVisibility) {
			...
			if (!WorldMapController.displayWorldMap(context, world))
				return;
			hide(false);
			...
			hide(false);
		} else if (btn == toolbox_twinsprite) {
			if (Dialogs.showTwinsprite((Activity) getContext(), controllers, world)) {
				hide(false);
			}
		}
	}
